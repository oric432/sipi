...
inline std::string pretty_function(std::string_view fn_full) {
  ...
  constexpr std::string_view kCvs[] = {"__cdecl", "__thiscall", "__stdcall",
  ...
  if (auto first_scope = str.find("::");
      first_scope != std::string_view::npos) {
...
public:
  ...
  [[nodiscard]] Error with_context(
    ...
    context_stack_.push_back(
        std::format("[{} @ {}] {}", function, where, additional_context));
...
inline Error
make_error(std::source_location location = std::source_location::current()) {
