...
void answer_and_send(pjsip_inv_session* inv, int code, const pjmedia_sdp_session* sdp) {
    // pjsip_inv_answer only builds the response tx_data. The separate send call
    // is what hands it to the transaction layer for retransmits and transport IO.
...
void SipResponder::send_trying(pjsip_inv_session* inv, pjsip_rx_data* rdata) {
    // The first provisional response still needs the original rx_data so PJSIP
    // can seed the UAS transaction correctly.
    pjsip_tx_data* tdata = nullptr;
...
void SipResponder::send_ok(pjsip_inv_session* inv, std::string_view sdp_body) {
    // SDP objects must live in a PJ pool. The dialog pool is a natural fit
    // because the answer belongs to this INVITE session.
